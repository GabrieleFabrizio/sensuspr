...
if (preg_match("/controller.php/",$_SERVER['PHP_SELF'])) {
  ...
  die();
    ...
    case "index":
        ...
        global $theme ;
        $theme['lenght'] = '700';
        $theme['titles'] = '["Sensus PR", "Enthusiasts", "Effective"]';
        $theme['subtitles'] = ' ["Ignite influential communications", "for your business"],
                                ["Feel. Connect. Empower.", "Expert storytelling for global brands"],
                                ["Cultivate lasting impact", "with strategic and creative excellence."]';
 
        //latest blogposts from db
        global $Blog;
  ...
  function getLatestBlogposts() {
    global $database;
    $query = "SELECT * FROM blog ORDER BY LAST_MODIFIED DESC, ID DESC LIMIT 3";
    $blogs = $database->db_query($query);
    return $database->db_fetch_allrows($blogs);
  }
